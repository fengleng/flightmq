...
func (c *Client) Push(pkg MsgPkg) ([]byte, error) {
	...
	if _, err := c.conn.Write(bodyLenBytes); err != nil {
		return nil, err
	}
...
// Set 设置topic配置
// set <topic> <is_auto_ack> <mode> <msgTTR> <msgRetry>
func (c *Client) Set(topic string, isAutoAck, mode, msgTTR, msgRetry int) ([]byte, error) {
	...
	params = append(params, []byte(strconv.Itoa(mode)))
	params = append(params, []byte(strconv.Itoa(msgTTR)))
	params = append(params, []byte(strconv.Itoa(msgRetry)))
	line := bytes.Join(params, []byte(" "))
	if _, err := c.conn.Write(line); err != nil {
		return nil, err
	}
...
func (c *Client) Ping() ([]byte, error) {
	var params [][]byte
	params = append(params, []byte("ping"))
	line := bytes.Join(params, []byte(" "))
	if _, err := c.conn.Write(line); err != nil {
		return nil, err
	}
